...
// Filter for PumpWallets group
const pumpWallets = walletsData.filter(wallet => wallet.group === 'PumpWallets');

console.log('=== PUMP WALLET GROUP PRIVATE KEYS ===\n');
console.log(`Found ${pumpWallets.length} wallets in PumpWallets group\n`);

// Display each wallet's information
pumpWallets.forEach((wallet, index) => {
...
const pumpWalletSummary = {
  groupName: 'PumpWallets',
  totalWallets: pumpWallets.length,
  wallets: pumpWallets.map(wallet => ({
    name: wallet.name,
...
fs.writeFileSync('./pump-wallets-keys.json', JSON.stringify(pumpWalletSummary, null, 2));

console.log('=== SUMMARY ===');
console.log(`Total Pump Wallets: ${pumpWallets.length}`);
console.log('Private keys have been saved to: pump-wallets-keys.json');
...
const privateKeysOnly = pumpWallets.map(wallet => 
  `${wallet.name}: ${bs58.encode(new Uint8Array(wallet.secretKey))}`
).join('\n');

fs.writeFileSync('./pump-private-keys.txt', privateKeysOnly);
console.log('\nPrivate keys only saved to: pump-private-keys.txt');
